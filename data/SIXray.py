...
class SIXrayAnnotationTransform(object):
    ...
    def __call__(self, target, width, height, idx):
        ...
        # 遍历Annotation
        # 按照自定义文件格式读取
        # root_annotation = '~/workspace/ml-project/datasets/anotation'
        res = []
...
class SIXrayDetection(data.Dataset):
    ...
    def __init__(self, root,
        ...
        # TODO: Anotation file format
        self._annopath = osp.join('%s' % self.root, 'Anno_core_coreless_battery_sub_3000_3000', '%s.txt')
        # self._imgpath = osp.join('%s' % self.root, 'Image', '%s.jpg')

        # TODO: image file format
        self._imgpath = osp.join('%s' % self.root, 'cut_Image_core_coreless_battery_sub_4000_3000', '%s.TIFF')
        ###这尼玛还有小写的tiff？
        self._imgpath1 = osp.join('%s' % self.root, 'cut_Image_core_coreless_battery_sub_3000_300', '%s.tiff')
        
        # TODO: image file format
        self._imgpath_jpg = osp.join('%s' % self.root, 'cut_Image_core_coreless_battery_sub_3000_3000', '%s.jpg')
        self.ids = list()
