...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num):
            ...
            match(self.threshold, predicts, truths, defaults, self.variance, labels, loc_t, loc_g, conf_t, idx)
        if self.use_gpu:
            loc_t = loc_t.cuda()
            loc_g = loc_g.cuda()
            conf_t = conf_t.cuda()
        ...
        # FIXED: fix the bug which multiply bug
        #loss_c = F.cross_entropy(conf_p, targets_weighted, reduction='sum')
